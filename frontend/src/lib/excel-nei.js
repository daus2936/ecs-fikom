// ============================================================
// Excel helpers untuk Nei.
// Struktur identik dengan Invoice Detail tapi pakai 7 field cover.
// ============================================================
import * as XLSX from 'xlsx';
import { NEI_NUMERIC_FIELDS } from './nei-constants.js';
...
for (const [k, label] of NEI_NUMERIC_FIELDS) {
...
function buildReferensiSheet() {
  ...
  ref.push(['PANDUAN PENGISIAN TEMPLATE NEI', '']);
  ref.push(['', '']);
  ref.push(['Cara pakai:', '']);
  ...
  ref.push(['', '']);
  ref.push(['Catatan:', 'Hanya client yang boleh. Internal KBSI / Internal SMI tidak berlaku di Nei.']);
  ref.push(['', '']);
  ...
  ref.push(['Catatan',                          'Nei TIDAK memiliki category4 (berbeda dengan Invoice Detail).']);
  ref.push(['', '']);
  ...
  for (const [k, label] of NEI_NUMERIC_FIELDS) ref.push([k, label]);
...
export function downloadTemplate() {
  ...
  XLSX.writeFile(wb, 'template-nei.xlsx');
}
...
export async function parseExcelFile(file) {
  ...
  return raw.map((r, idx) => {
    ...
    for (const [k] of NEI_NUMERIC_FIELDS) {
      const v = r[k];
