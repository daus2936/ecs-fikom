...
export default function BayarHutang() {
  ...
  const role = me?.role;
  const canCreate = pCanCreate(role, 'bayar-hutang');
  const canEdit = pCanEdit(role, 'bayar-hutang');
  const canDelete = pCanDelete(role, 'bayar-hutang');
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/bayar-hutang', { params });
      setItems(data.bayar_hutang);
  ...
  const handleDelete = async (it) => {
    if (!confirm(`Hapus pembayaran hutang ${it.kode_hutang}?`)) return;
    try {
      await api.delete(`/bayar-hutang/${it.id}`);
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Bayar Hutang</h1>
          <p className="text-slate-500 mt-1 text-sm">Pembayaran pokok hutang. Cari berdasarkan kode hutang.</p>
        </div>
        <div className="flex items-center gap-2">
          {isAdmin(role) && (
            <ExportButton
              endpoint="/bayar-hutang"
              dataKey="bayar_hutang"
              filename="bayar-hutang"
              params={{
              ...
              columns={[
                ...
                { header: 'Periode Pelunasan (hari)', value: (r) => r.periode_pelunasan_hari },
          ...
          {canCreate && <button onClick={() => setEditing('new')} className="btn-primary">+ Tambah Bayar Hutang</button>}
        </div>
      </div>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-emerald-800">
          <span className="font-medium">Total Nominal Bayar Hutang</span>
          {(dateFrom || dateTo) && (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-600 text-xs uppercase tracking-wide">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium">Periode Lunas</th>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && pageItems.map((it) => (
                <tr key={it.id} className="hover:bg-slate-50 align-top">
                  ...
                  <td className="px-4 py-3 text-right text-slate-700 whitespace-nowrap">{it.periode_pelunasan_hari} hari</td>
...
const EMPTY = {
  ...
  nilai_bayar: 0,
  periode_pelunasan_hari: 30,
  hutang_id: null,
  rekening_masuk_id: null,
  rekening_dari_id: null,
};

function BayarHutangViewModal({ item, onClose, onEdit }) {
  if (!item) return null;
  const wasEdited = item.updated_at && item.updated_at !== item.created_at;
  return (
    <Modal open={!!item} onClose={onClose} title={`Detail Bayar Hutang · ${item.kode_hutang || '—'}`} maxWidth="max-w-2xl">
      <div className="space-y-5">
        ...
        <section>
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            ...
            <BHField label="Kode Hutang"   value={<span className="font-mono">{item.kode_hutang || '—'}</span>} />
            <BHField label="Nilai Bayar"   value={<span className="font-mono font-medium">{formatIDR(item.nilai_bayar)}</span>} />
            <BHField label="Periode Pelunasan" value={`${item.periode_pelunasan_hari} hari`} />
...
function BayarHutangFormModal({ editing, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    setForm(isEdit ? {
      ...
      periode_pelunasan_hari: Number(editing.periode_pelunasan_hari) || 30,
  ...
  const submit = async (e) => {
    ...
    if (!(form.periode_pelunasan_hari > 0)) return setError('Periode pelunasan harus > 0 hari.');
    if (!form.hutang_id) return setError('Pilih kode hutang yang dibayar.');
    ...
    try {
      const payload = {
        ...
        periode_pelunasan_hari: Number(form.periode_pelunasan_hari),
        hutang_id: form.hutang_id,
        rekening_masuk_id: form.rekening_masuk_id,
        rekening_dari_id: form.rekening_dari_id,
      };
      if (isEdit) await api.put(`/bayar-hutang/${editing.id}`, payload);
      else        await api.post('/bayar-hutang', payload);
      onSaved();
  ...
  if (!open) return null;
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Bayar Hutang' : 'Tambah Bayar Hutang'} maxWidth="max-w-2xl">
      <form onSubmit={submit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="label">Tanggal Bayar Hutang <span className="text-red-500">*</span></label>
            <input type="date" required className="input"
              value={form.tanggal_bayar}
              onChange={(e) => setForm((f) => ({ ...f, tanggal_bayar: e.target.value }))} />
          </div>
          <div>
            <label className="label">Periode Pelunasan (Hari) <span className="text-red-500">*</span></label>
            <input type="number" required min="1" className="input"
              value={form.periode_pelunasan_hari}
              onChange={(e) => setForm((f) => ({ ...f, periode_pelunasan_hari: e.target.value }))} />
          </div>
        </div>
        <div>
          <label className="label">Nilai Bayar Hutang (Rupiah) <span className="text-red-500">*</span></label>
