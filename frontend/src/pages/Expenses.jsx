...
import { downloadExpenseTemplate, parseExpenseExcel } from '../lib/excel-expense.js';
...
export default function Expenses() {
  ...
  }, [fSubEntities, fCategory1, fCategory3, fPoIds, fInvoiceIds, dateFrom, dateTo, sortDir, load]);

  const hasActiveFilter = Boolean(
    fSubEntities.length || fCategory1.length || fCategory3.length ||
  ...
  const handleDelete = async (it) => {
    ...
    try {
      await api.delete(`/expenses/${it.id}`);
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post('/expenses/import', { rows });
    ...
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Daftar expense. Semua user bisa lihat dan edit.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {isAdmin(role) && (
            <ExportButton
              ...
              columns={[
                { header: 'Kode Expense', value: (r) => r.expense_code },
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-emerald-800">
          <span className="font-medium">Total Nominal Expenses</span>
          {(dateFrom || dateTo) && (
      ...
      {showSelect && (
        <BulkDeleteBar
          ...
          label="Expenses"
          onClear={bulk.clear}
          onDone={() => { bulk.clear(); load({ sub_entities: fSubEntities, category1: fCategory1, category3: fCategory3, po_ids: fPoIds, invoice_ids: fInvoiceIds, date_from: dateFrom, date_to: dateTo, sort: sortDir }); }}
        />
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading && (
                <tr><td colSpan={showSelect ? 10 : 9} className="px-4 py-10 text-center text-slate-400">Memuat…</td></tr>
              )}
              {!loading && items.length === 0 && (
                <tr><td colSpan={showSelect ? 10 : 9} className="px-4 py-12 text-center text-slate-400">
...
function ExpenseFormModal({ editing, onClose, onSaved }) {
  ...
  const setExpenseType = (type) => {
    setForm((f) => {
      ...
      return {
        ...
        category3: '',
        purchase_order_ids: type === 'client' ? f.purchase_order_ids : [],
        invoice_ids:        type === 'client' ? f.invoice_ids        : [],
      };
    });
  };
  ...
  // ----- Validation -----
  const validate = () => {
    if (!form.occurred_date)         return 'Tanggal terjadi expense wajib diisi.';
    ...
    if (form.expense_type === 'client' && form.purchase_order_ids.length === 0) {
  ...
  const submit = async (e) => {
    ...
    const isClient = form.expense_type === 'client';
    ...
    const payload = {
      ...
      purchase_order_ids: isClient ? form.purchase_order_ids : [],
      invoice_ids:        isClient ? form.invoice_ids        : [],
    };
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Expense' : 'Tambah Expense'} maxWidth="max-w-2xl">
      <form onSubmit={submit} className="space-y-5">
        ...
        {/* 8 & 9. PO + Invoice refs — hanya untuk Client */}
  ...
  return (
    <Modal open={!!item} onClose={onClose} title="Detail Expense" maxWidth="max-w-2xl">
      <div className="space-y-5">
        ...
        {/* Referensi (hanya untuk client) */}
        {item.expense_type === 'client' && (
          <section className="pt-4 border-t border-slate-100">
