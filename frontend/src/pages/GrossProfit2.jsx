...
// Halaman utama
// =====================================================
export default function GrossProfit2() {
  ...
  const calculate = async (e) => {
    ...
    try {
      const endpoint = mode === 'period'
        ? '/gross-profit-2/calculate'
        : '/gross-profit-2/calculate-by-invoice';
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Gross Profit 2</h1>
        <p className="text-slate-500 mt-1 text-sm">
          Kalkulasi Gross Profit 2 = Total Payment (yang merujuk invoice di Invoice Cover) − Total Expense (yang merujuk invoice di Payment).
        </p>
      </div>
        ...
        <div className="space-y-5">
          ...
          {mode === 'invoice' && (
            <div>
              ...
              <p className="text-xs text-slate-400 mt-2">
                ...
                lalu cari Payment yang merujuk invoice tsb, dan kurangi dengan Expense yang merujuk invoice di Payment.
              </p>
            </div>
          )}
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-slate-100">
            ...
            <button type="submit" disabled={!canCalculate || loading} className="btn-primary">
              {loading ? 'Menghitung…' : 'Hitung Gross Profit 2'}
            </button>
          </div>
        </div>
      </form>
...
// =====================================================
function exportGP2(data) {
  const { filters, totals, payments, expenses } = data;
  ...
  const ringkasan = [
    ...
    { label: 'Total Payment', value: Number(totals.payment_total) },
    { label: 'Total Expense', value: Number(totals.expense_total) },
    { label: 'Gross Profit 2 (Payment − Expense)', value: Number(totals.gross_profit_2) },
    { label: '% Gross Profit 2 (GP2 / Total Payment)', value: totals.gross_profit_2_percent === null ? '—' : Number(totals.gross_profit_2_percent) },
    { label: 'Jumlah Payment', value: payments.length },
    { label: 'Jumlah Expense', value: expenses.length },
  ];

  exportSheetsToExcel('gross-profit-2', [
    {
      name: 'Ringkasan',
      columns: [
    ...
    },
    {
      name: 'Payment',
      columns: [
        { header: 'Tanggal', value: (p) => formatDateOnly(p.payment_date) },
        { header: 'Client', value: (p) => subEntityLabel(p.sub_entity) },
        { header: 'Kategori 3', value: (p) => (p.category3 ? cat3Label(p.category3) : '') },
        { header: 'Nominal', value: (p) => Number(p.amount) },
        { header: 'Invoice', value: (p) => (p.invoices || []).map((i) => i.invoice_number).join(', ') },
      ],
      rows: payments,
    },
    {
...
function ResultView({ data }) {
  const { filters, totals, invoice_covers: covers, payments, expenses } = data;
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between gap-3 mb-3 flex-wrap">
          ...
          <button type="button" onClick={() => exportGP2(data)} className="btn-secondary">
            Export Excel
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <SummaryCard
            label="Total Payment"
            value={formatIDR(totals.payment_total)}
            sublabel={`${payments.length} payment`}
          />
          <SummaryCard
            label="Total Expense"
          ...
          />
          <SummaryCard
            label="Gross Profit 2"
            value={formatIDR(totals.gross_profit_2)}
            highlight={totals.gross_profit_2 >= 0 ? 'positive' : 'negative'}
            sublabel="Payment − Expense"
          />
          <SummaryCard
            label="% Gross Profit 2"
            value={formatPercent(totals.gross_profit_2_percent)}
            highlight={
              totals.gross_profit_2_percent === null
                ? null
                : totals.gross_profit_2_percent >= 0 ? 'positive' : 'negative'
            }
            sublabel="GP2 / Total Payment"
          />
        </div>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
        ...
        <div className="text-sm space-y-1 font-mono">
          <div className="flex items-baseline gap-2">
            <span className="text-slate-500 w-44">Total Payment</span>
            <span>=</span>
            <span className="font-medium">{formatIDR(totals.payment_total)}</span>
          </div>
          <div className="flex items-baseline gap-2">
          ...
          <div className="flex items-baseline gap-2 pt-1 border-t border-slate-300">
            <span className="text-slate-500 w-44">Gross Profit 2</span>
            <span>=</span>
            <span className="font-medium">
              {formatIDR(totals.payment_total)} − {formatIDR(totals.expense_total)} = <span className="text-slate-900 font-bold">{formatIDR(totals.gross_profit_2)}</span>
            </span>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-slate-500 w-44">% Gross Profit 2</span>
            ...
            <span className="font-medium">
              {totals.payment_total > 0
                ? <>{formatIDR(totals.gross_profit_2)} ÷ {formatIDR(totals.payment_total)} = <span className="text-slate-900 font-bold">{formatPercent(totals.gross_profit_2_percent)}</span></>
                : <span className="text-slate-500 italic">Tidak bisa dihitung (Total Payment = 0)</span>}
            </span>
          </div>
        </div>
      </div>
      ...
      {/* Detail tables — 3 kolom: Cover → Payment → Expense */}
...
function PaymentList({ payments }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 bg-slate-50">
        <h3 className="font-medium text-slate-900">Payment yang Dihitung</h3>
        <p className="text-xs text-slate-500 mt-0.5">
          {payments.length} payment · Total {formatIDR(total)}
        </p>
      </div>
      <div className="overflow-x-auto max-h-96">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {payments.length === 0 && (
              <tr><td colSpan={4} className="px-3 py-8 text-center text-slate-400">Tidak ada Payment yang merujuk ke invoice di Invoice Cover yang dipilih.</td></tr>
            )}
            {payments.map((p) => (
              <tr key={p.id} className="align-top">
                <td className="px-3 py-2 whitespace-nowrap text-slate-700">{formatDateOnly(p.payment_date)}</td>
                <td className="px-3 py-2 text-slate-700">
                  <div>{subEntityLabel(p.sub_entity)}</div>
                  {p.category3 && <div className="text-xs text-slate-400">{cat3Label(p.category3)}</div>}
                </td>
                <td className="px-3 py-2 text-right font-mono whitespace-nowrap text-slate-900">{formatIDR(p.amount)}</td>
                <td className="px-3 py-2">
                  {p.invoices.length === 0
                    ? <span className="text-slate-300">—</span>
                    : (
