...
import { INVOICE_COVER_NUMERIC_FIELDS } from '../lib/invoice-cover-constants.js';
import { downloadTemplate, parseExcelFile } from '../lib/excel-invoice-cover.js';
...
// Halaman utama
// =====================================================
export default function InvoiceCoverBefore2025() {
  const { user: me } = useAuth();
  const role = me?.role;
  const canCreate = pCanCreate(role, 'invoice-covers');
  const canEdit = pCanEdit(role, 'invoice-covers');
  const canDelete = pCanDelete(role, 'invoice-covers');
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/invoice-cover-before-2025', { params });
      setItems(data.invoice_cover_before_2025);
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get('/invoices'),
      api.get('/invoice-cover-before-2025/faktur-pajak-options'),
    ])
  ...
  }, [fSubEntities, fCategory3, fFakturPajak, fPoIds, fInvoiceIds, dateFrom, dateTo, load]);
  ...
  const handleDelete = async (it) => {
    const msg =
      `Yakin ingin menghapus Invoice Cover ini?\n\n` +
    ...
    try {
      await api.delete(`/invoice-cover-before-2025/${it.id}`);
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post('/invoice-cover-before-2025/import', { rows });
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Invoice Cover Before 2025</h1>
        ...
        <div className="flex gap-2">
          ...
          <input
            ...
            className="hidden"
          />
          {canCreate && <button onClick={() => setEditing('new')} className="btn-primary">+ Tambah Invoice Cover</button>}
        </div>
      </div>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-emerald-800">
          <span className="font-medium">Total Nominal Invoice Cover</span>
          {(dateFrom || dateTo) && (
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {!loading && items.length === 0 && (
                <tr><td colSpan={8} className="px-4 py-12 text-center text-slate-400">
                  {hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data. Klik "Tambah Invoice Cover" atau import Excel.'}
                </td></tr>
              )}
...
// =====================================================
// Modal: Tambah/Edit Invoice Cover
// =====================================================
const EMPTY_FORM = {
  ...
  purchase_order_ids: [],
  invoice_ids: [],
  ...Object.fromEntries(INVOICE_COVER_NUMERIC_FIELDS.map(([k]) => [k, 0])),
};

function InvoiceCoverFormModal({ editing, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      for (const [k] of INVOICE_COVER_NUMERIC_FIELDS) next[k] = Number(editing[k]) || 0;
      setForm(next);
    } else {
      setForm(EMPTY_FORM);
    }
  ...
  const submit = async (e) => {
    ...
    for (const [k] of INVOICE_COVER_NUMERIC_FIELDS) payload[k] = Number(form[k] || 0);

    setSubmitting(true);
    try {
      if (isEdit) await api.put(`/invoice-cover-before-2025/${editing.id}`, payload);
      else        await api.post('/invoice-cover-before-2025', payload);
      onSaved();
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
  ...
  return (
    <Modal open={open} onClose={onClose}
      title={isEdit ? 'Edit Invoice Cover' : 'Tambah Invoice Cover'}
      maxWidth="max-w-3xl">
      <form onSubmit={submit} className="space-y-5">
        ...
        <div className="pt-2 border-t border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-3">
            {INVOICE_COVER_NUMERIC_FIELDS.map(([key, label]) => (
              <NumericInput
                key={key}
                label={label}
                value={form[key]}
...
// =====================================================
function InvoiceCoverViewModal({ item, onClose, onEdit }) {
  if (!item) return null;

  return (
    <Modal open={!!item} onClose={onClose} title="Detail Invoice Cover" maxWidth="max-w-3xl">
