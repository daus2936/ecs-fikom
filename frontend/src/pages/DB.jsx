...
import { downloadDBTemplate, parseDBExcel } from '../lib/excel-db.js';
...
export default function DB() {
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/db', { params });
      setItems(data.db);
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      const { rows } = await parseDBExcel(file);
      if (rows.length === 0) {
      ...
      const { data } = await api.post('/db/import', { rows });
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">DB</h1>
          <p className="text-slate-500 mt-1 text-sm">
            Daftar DB. Hanya admin/superadmin.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={downloadDBTemplate}
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-emerald-800">
          <span className="font-medium">Total Nominal DB</span>
          {(dateFrom || dateTo) && (
...
function ExpenseFormModal({ editing, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/db/${editing.id}`, payload);
      } else {
        await api.post('/db', payload);
      }
      onSaved();
    } catch (err) {
      setError(getErrorMessage(err));
