...
import {
  CATEGORY3, CATEGORY4,
  getClientOptions, subEntityLabel, cat3Label, cat4Label,
} from '../lib/expense-constants.js';
import { INVOICE_DETAIL_NUMERIC_FIELDS } from '../lib/invoice-detail-constants.js';
import { downloadTemplate, parseExcelFile } from '../lib/excel.js';
...
export default function InvoiceDetails() {
  ...
  const role = me?.role;
  const canCreate = pCanCreate(role, 'invoice-details');
  const canEdit = pCanEdit(role, 'invoice-details');
  const canDelete = pCanDelete(role, 'invoice-details');
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/invoice-details', { params });
      setItems(data.invoice_details);
  ...
  // Filter aktif → dipakai juga saat refresh sesudah create/import
  ...
  }, [fSubEntities, fCategory3, fPoIds, fInvoiceIds, dateFrom, dateTo, sortDir, load]);

  const hasActiveFilter = Boolean(
    fSubEntities.length || fCategory3.length || fPoIds.length || fInvoiceIds.length || dateFrom || dateTo
  );
  const resetFilters = () => {
    setFSubEntities([]); setFCategory3([]); setFPoIds([]); setFInvoiceIds([]); setDateFrom(''); setDateTo('');
  };

  const handleDelete = async (it) => {
    const total = INVOICE_DETAIL_NUMERIC_FIELDS.reduce((s, [k]) => s + Number(it[k] || 0), 0);
    const msg =
      `Yakin ingin menghapus Invoice Detail ini?\n\n` +
    ...
    try {
      await api.delete(`/invoice-details/${it.id}`);
  ...
  const handleFileChange = async (e) => {
    ...
    e.target.value = ''; // reset biar bisa pilih file sama dua kali
    if (!file) return;
    ...
    try {
      ...
      const { data } = await api.post('/invoice-details/import', { rows });
    ...
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Detail invoice per client. Bisa input langsung atau import dari Excel.
          </p>
        </div>
          ...
          {isAdmin(role) && (
            <ExportButton
              ...
              columns={[
                ...
                { header: 'Kategori 4', value: (r) => r.category4 || '' },
                ...
                ...INVOICE_DETAIL_NUMERIC_FIELDS.map(([key, label]) => ({
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-emerald-800">
          <span className="font-medium">Total Nominal Invoice Detail</span>
          {(dateFrom || dateTo) && (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-600 text-xs uppercase tracking-wide">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium">Total Nominal</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading && (
                <tr><td colSpan={showSelect ? 8 : 7} className="px-4 py-10 text-center text-slate-400">Memuat…</td></tr>
              )}
              {!loading && items.length === 0 && (
                <tr><td colSpan={showSelect ? 8 : 7} className="px-4 py-12 text-center text-slate-400">
                  {hasActiveFilter ? 'Tidak ada data yang cocok dengan filter.' : 'Belum ada data. Klik "Tambah Invoice Detail" atau import Excel.'}
                </td></tr>
              )}
                ...
                return (
                  ...
                  {showSelect && (<td className="px-4 py-3"><input type="checkbox" checked={bulk.isSelected(it.id)} onChange={() => bulk.toggle(it.id)} /></td>)}
                    ...
                    <td className="px-4 py-3 text-slate-600">
                      {it.category3 && <div>{it.category3}</div>}
                      {it.category4 && <div className="text-xs text-slate-500">↳ {it.category4}</div>}
                      {!it.category3 && !it.category4 && <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-4 py-3 text-right font-mono whitespace-nowrap text-slate-900">{formatIDR(total)}</td>
...
// =====================================================
// Modal: Tambah/Edit Invoice Detail
// =====================================================
const EMPTY_FORM = {
  ...
  purchase_order_ids: [],
  invoice_ids: [],
  ...Object.fromEntries(INVOICE_DETAIL_NUMERIC_FIELDS.map(([k]) => [k, 0])),
};

function InvoiceDetailFormModal({ editing, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      for (const [k] of INVOICE_DETAIL_NUMERIC_FIELDS) next[k] = Number(editing[k]) || 0;
      setForm(next);
    } else {
      setForm(EMPTY_FORM);
    }
  ...
  const showCategory4 = showCategory3 && form.category3 === 'NMA';

  // Cascading: ganti client → reset cat3+cat4. Ganti cat3 → reset cat4.
  const setSubEntity = (code) => {
    setForm((f) => ({ ...f, sub_entity: code, category3: '', category4: '' }));
  };
  const setCategory3 = (code) => {
    setForm((f) => ({ ...f, category3: code, category4: '' }));
  };

  const total = useMemo(
    () => INVOICE_DETAIL_NUMERIC_FIELDS.reduce((s, [k]) => s + Number(form[k] || 0), 0),
    [form]
  );
  ...
  const validate = () => {
    ...
    // Cat3 wajib kalau BIERSDORF
    if (form.sub_entity === 'BIERSDORF' && !form.category3) {
      return 'Kategori 3 (NMA/BMC/KPL) wajib diisi untuk PT Biersdorf Indonesia.';
    }
    // Cat4 wajib kalau cat3 = NMA
    if (form.category3 === 'NMA' && !form.category4) {
      return 'Kategori 4 (NMA/TL) wajib diisi kalau Kategori 3 = NMA.';
    }
    if (form.category4 && form.category3 !== 'NMA') {
      return 'Kategori 4 hanya berlaku kalau Kategori 3 = NMA.';
    }
    return null;
  };
  ...
  const submit = async (e) => {
    ...
    const payload = {
      ...
      category4:   showCategory4 && form.category4 ? form.category4 : null,
      purchase_order_ids: form.purchase_order_ids,
      invoice_ids:        form.invoice_ids,
    };
    for (const [k] of INVOICE_DETAIL_NUMERIC_FIELDS) payload[k] = Number(form[k] || 0);

    setSubmitting(true);
    try {
      if (isEdit) {
        await api.put(`/invoice-details/${editing.id}`, payload);
      } else {
        await api.post('/invoice-details', payload);
      }
      onSaved();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  return (
    <Modal open={open} onClose={onClose}
      title={isEdit ? 'Edit Invoice Detail' : 'Tambah Invoice Detail'}
      maxWidth="max-w-3xl">
      <form onSubmit={submit} className="space-y-5">
        ...
        {showCategory3 && (
          <div className="grid grid-cols-2 gap-4">
            ...
            {showCategory4 && (
              <div>
                <label className="label">Kategori 4 <span className="text-red-500">*</span></label>
                <select required className="input"
                  value={form.category4}
                  onChange={(e) => setForm((f) => ({ ...f, category4: e.target.value }))}>
                  <option value="">— Pilih —</option>
                  {Object.entries(CATEGORY4).map(([code, label]) => (
                    <option key={code} value={code}>{label}</option>
                  ))}
                </select>
              </div>
        ...
        <div className="pt-2 border-t border-slate-100">
          ...
          <div className="mt-3 text-right text-sm">
            ...
            <span className="font-mono font-medium text-slate-900">{formatIDR(total)}</span>
          </div>
        </div>
...
// =====================================================
function InvoiceDetailViewModal({ item, onClose, onEdit }) {
  if (!item) return null;

  const total = INVOICE_DETAIL_NUMERIC_FIELDS.reduce((s, [k]) => s + Number(item[k] || 0), 0);

  return (
    <Modal open={!!item} onClose={onClose} title="Detail Invoice" maxWidth="max-w-3xl">
      <div className="space-y-5">
        ...
        <section>
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            ...
            {item.category4 && <Field label="Kategori 4" value={cat4Label(item.category4)} />}
          </dl>
        </section>

        {/* Section: Referensi PO & Invoice */}
          ...
          <div className="bg-slate-50 rounded-lg border border-slate-200 divide-y divide-slate-200">
            {INVOICE_DETAIL_NUMERIC_FIELDS.map(([key, label]) => (
              <div key={key} className="flex items-center justify-between px-4 py-2.5 text-sm">
                ...
                <span className="font-mono text-slate-900">{formatIDR(item[key])}</span>
              </div>
            ))}
