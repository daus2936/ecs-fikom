...
// Halaman utama
// =====================================================
export default function InvoiceCovers() {
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/invoice-covers', { params });
      setItems(data.invoice_covers);
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get('/invoices'),
      api.get('/invoice-covers/faktur-pajak-options'),
    ])
  ...
  const handleDelete = async (it) => {
    ...
    try {
      await api.delete(`/invoice-covers/${it.id}`);
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post('/invoice-covers/import', { rows });
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Invoice Cover</h1>
        ...
        <div className="flex gap-2">
          ...
          {isAdmin(role) && (
            <ExportButton
              endpoint="/invoice-covers"
              dataKey="invoice_covers"
              filename="invoice-cover"
              params={{
...
function InvoiceCoverFormModal({ editing, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true);
    try {
      if (isEdit) await api.put(`/invoice-covers/${editing.id}`, payload);
      else        await api.post('/invoice-covers', payload);
      onSaved();
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
