...
import { downloadExpensesBefore2025Template, parseExpensesBefore2025Excel } from '../lib/excel-expenses-before-2025.js';

// =====================================================
// Halaman utama
// =====================================================
export default function ExpensesBefore2025() {
  ...
  const load = useCallback(async (filters = {}) => {
    ...
    try {
      ...
      const { data } = await api.get('/expenses-before-2025', { params });
      setItems(data.expenses_before_2025);
  ...
  }, [fSubEntities, fCategory1, fCategory3, fPoIds, fInvoiceIds, dateFrom, dateTo, load]);
  ...
  const handleDelete = async (it) => {
    ...
    try {
      await api.delete(`/expenses-before-2025/${it.id}`);
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      const { rows } = await parseExpensesBefore2025Excel(file);
      if (rows.length === 0) {
      ...
      const { data } = await api.post('/expenses-before-2025/import', { rows });
  ...
  return (
    <div className="p-8 max-w-7xl">
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Expenses Before 2025</h1>
          <p className="text-slate-500 mt-1 text-sm">
            Daftar expenses sebelum 2025. Hanya admin/superadmin.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={downloadExpensesBefore2025Template}
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
        <div className="text-sm text-emerald-800">
          <span className="font-medium">Total Nominal Expenses Before 2025</span>
          {(dateFrom || dateTo) && (
...
function ExpenseFormModal({ editing, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      if (isEdit) {
        await api.put(`/expenses-before-2025/${editing.id}`, payload);
      } else {
        await api.post('/expenses-before-2025', payload);
      }
      onSaved();
    } catch (err) {
      setError(getErrorMessage(err));
