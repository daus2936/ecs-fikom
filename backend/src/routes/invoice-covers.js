...
async function replacePivot(client, table, fkColumn, parentId, ids) {
  await client.query(`DELETE FROM ${table} WHERE invoice_cover_id = $1`, [parentId]);
  if (ids.length === 0) return;
  const placeholders = ids.map((_, i) => `($1, $${i + 2})`).join(', ');
  await client.query(
    `INSERT INTO ${table} (invoice_cover_id, ${fkColumn}) VALUES ${placeholders}`,
    [parentId, ...ids]
  );
}
...
function buildWhereMulti({ subEntities, category3, nomorFaktur, poIds, invoiceIds, dateFrom, dateTo }) {
  ...
  if (Array.isArray(poIds) && poIds.length > 0) {
    params.push(poIds);
    where.push(`EXISTS (SELECT 1 FROM invoice_cover_purchase_orders icpo
                WHERE icpo.invoice_cover_id = c.id AND icpo.purchase_order_id = ANY($${params.length}::int[]))`);
  }
  if (Array.isArray(invoiceIds) && invoiceIds.length > 0) {
    params.push(invoiceIds);
    where.push(`EXISTS (SELECT 1 FROM invoice_cover_invoices ici
                WHERE ici.invoice_cover_id = c.id AND ici.invoice_id = ANY($${params.length}::int[]))`);
  }
...
// SUM per kolom numeric (total nominal masing-masing variabel),
// tanpa LIMIT, ikut filter. Mengembalikan satu baris berisi
// kolom sum_<key> untuk tiap variabel + cnt.
function buildSumQuery(filters) {
  const { where, params } = buildWhereMulti(filters);
  const sumExprs = NUMERIC_KEYS.map((k) => `COALESCE(SUM(c.${k}),0) AS sum_${k}`).join(', ');
  const sql = `
    SELECT ${sumExprs}, COUNT(*) AS cnt
    FROM invoice_covers c
...
function buildListQuery(filters, noLimit = false, sort = null) {
  ...
  let orderBy = 'c.submit_date DESC, c.created_at DESC';
  if (sort === 'invoice_asc' || sort === 'invoice_desc') {
    const dir = sort === 'invoice_desc' ? 'DESC' : 'ASC';
    orderBy = `(SELECT MIN(i.invoice_number) FROM invoice_cover_invoices ici
                JOIN invoices i ON i.id = ici.invoice_id
                WHERE ici.invoice_cover_id = c.id) ${dir} NULLS LAST, c.created_at DESC`;
  }
  const sql = `
    SELECT c.id, c.submit_date, c.sub_entity, c.category3, c.nomor_faktur_pajak,
           ...
           COALESCE((SELECT json_agg(json_build_object('id', po.id, 'po_number', po.po_number) ORDER BY po.po_number)
                     ...
                     WHERE icpo.invoice_cover_id = c.id), '[]'::json) AS purchase_orders,
           COALESCE((SELECT json_agg(json_build_object('id', i.id, 'invoice_number', i.invoice_number) ORDER BY i.invoice_number)
                     ...
                     JOIN invoices i ON i.id = ici.invoice_id
                     WHERE ici.invoice_cover_id = c.id), '[]'::json) AS invoices
    FROM invoice_covers c
...
async function insertOne(client, data, userId) {
  ...
  const id = rows[0].id;
  await replacePivot(client, 'invoice_cover_purchase_orders', 'purchase_order_id', id, data.purchase_order_ids);
  await replacePivot(client, 'invoice_cover_invoices',        'invoice_id',        id, data.invoice_ids);
  return id;
}
...
// Routes
// ============================================================
export default async function invoiceCoverRoutes(fastify) {
  ...
  const adminOnly = { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'superadmin'])] };
    const canViewPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('invoice-covers', 'view')] };
const canCreate = { preHandler: [fastify.authenticate, fastify.requirePermission('invoice-covers', 'create')] };
  const canEditPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('invoice-covers', 'edit')] };
  const canDeletePerm = { preHandler: [fastify.authenticate, fastify.requirePermission('invoice-covers', 'delete')] };
  ...
  fastify.get('/', canViewPerm, async (request, reply) => {
    ...
    // Total nominal PER variabel (bukan grand total gabungan).
    // summary.totals = { total_biaya: ..., fee: ..., ... }
    const totals = {};
    for (const k of NUMERIC_KEYS) {
      totals[k] = Number(sumRows[0]?.[`sum_${k}`] || 0);
    }
    const summary = {
      totals,
    ...
    return { invoice_covers: rows, summary };
  });
  ...
  fastify.put('/:id', canEditPerm, async (request, reply) => {
    ...
    try {
      ...
      const { rowCount } = await client.query(
        `UPDATE invoice_covers SET ${setCols} WHERE id = $${idIdx}`,
        params
      );
      if (rowCount === 0) {
  ...
  fastify.delete('/:id', canDeletePerm, async (request, reply) => {
    const id = Number(request.params.id);
    if (!Number.isInteger(id) || id <= 0) return reply.code(400).send({ error: 'ID tidak valid.' });
    const { rowCount } = await query(`DELETE FROM invoice_covers WHERE id = $1`, [id]);
