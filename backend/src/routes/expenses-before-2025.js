...
async function generateExpenseCode(client) {
  ...
  const prefix = `EXPB-${ddmmyy}-`;

  // Advisory lock per-hari (key berbasis prefix tanggal).
  await client.query(`SELECT pg_advisory_xact_lock(hashtext($1))`, [`expensesb2025_code:${ddmmyy}`]);

  // Cari counter tertinggi untuk prefix hari ini.
  const { rows } = await client.query(
    `SELECT COALESCE(MAX(
       (regexp_replace(expense_code, '^EXPB-\\d{6}-', ''))::int
     ), 0) AS max_n
     FROM expenses_before_2025
...
function validateConsistency(data) {
  ...
  // Tidak ada aturan "client wajib PO". Pivot non_client juga boleh diisi.
...
async function replacePivot(client, table, fkColumn, expenseId, ids) {
  await client.query(`DELETE FROM ${table} WHERE eb_id = $1`, [expenseId]);
  ...
  await client.query(
    `INSERT INTO ${table} (eb_id, ${fkColumn}) VALUES ${placeholders}`,
    params
  );
}
...
function buildWhere({ subEntities, poIds, invoiceIds, category1, category3, dateFrom, dateTo }) {
  ...
  if (Array.isArray(poIds) && poIds.length > 0) {
    params.push(poIds);
    where.push(`EXISTS (
      SELECT 1 FROM eb2025_purchase_orders epo
      WHERE epo.eb_id = e.id AND epo.purchase_order_id = ANY($${params.length}::int[])
    )`);
  }
  ...
  if (Array.isArray(invoiceIds) && invoiceIds.length > 0) {
    ...
    where.push(`EXISTS (
      ...
      WHERE ei.eb_id = e.id AND ei.invoice_id = ANY($${params.length}::int[])
    )`);
  }
...
// Routes
// ============================================================
export default async function expensesBefore2025Routes(fastify) {
  const authOnly = { preHandler: [fastify.authenticate] };
    const canViewPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('expenses-before-2025', 'view')] };
const canCreate = { preHandler: [fastify.authenticate, fastify.requirePermission('expenses-before-2025', 'create')] };
  const canEditPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('expenses-before-2025', 'edit')] };
  const canDeletePerm = { preHandler: [fastify.authenticate, fastify.requirePermission('expenses-before-2025', 'delete')] };
  ...
  fastify.get('/', canViewPerm, async (request, reply) => {
    ...
    return { expenses_before_2025: rows, summary };
  });
  ...
  fastify.post('/', canCreate, async (request, reply) => {
    ...
    try {
      ...
      const { rows } = await client.query(
        `INSERT INTO expenses_before_2025
      ...
      await replacePivot(client, 'eb2025_purchase_orders', 'purchase_order_id', newId, data.purchase_order_ids);
      await replacePivot(client, 'eb2025_invoices',        'invoice_id',        newId, data.invoice_ids);
  ...
  fastify.put('/:id', canEditPerm, async (request, reply) => {
    ...
    try {
      ...
      const { rowCount } = await client.query(
        `UPDATE expenses_before_2025 SET
           occurred_date  = $1,
           expense_type   = $2,
           parent_company = $3,
           sub_entity     = $4,
      ...
      await replacePivot(client, 'eb2025_purchase_orders', 'purchase_order_id', id, data.purchase_order_ids);
      await replacePivot(client, 'eb2025_invoices',        'invoice_id',        id, data.invoice_ids);
      await client.query('COMMIT');
      return { id };
    } catch (e) {
      await client.query('ROLLBACK');
  ...
  // ----- DELETE /expenses/:id -------------------------------
  // Hanya admin & superadmin. Pivot tables (eb2025_purchase_orders,
  // eb2025_invoices) sudah ON DELETE CASCADE dari sisi expense,
  ...
  fastify.delete('/:id', canDeletePerm, async (request, reply) => {
    ...
    const { rowCount } = await query(`DELETE FROM expenses_before_2025 WHERE id = $1`, [id]);
    if (rowCount === 0) {
      return reply.code(404).send({ error: 'Expense tidak ditemukan.' });
    }
    return { success: true };
    ...
    try {
      ...
      for (const r of validated) {
        ...
        if (r._poIds.length > 0) {
          await replacePivot(client, 'eb2025_purchase_orders', 'purchase_order_id', newId, r._poIds);
        }
        if (r._invIds.length > 0) {
          await replacePivot(client, 'eb2025_invoices', 'invoice_id', newId, r._invIds);
        }
      }
      await client.query('COMMIT');
