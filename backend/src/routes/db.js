...
async function generateExpenseCode(client) {
  ...
  const prefix = `DB-${ddmmyy}-`;

  // Advisory lock per-hari (key berbasis prefix tanggal).
  await client.query(`SELECT pg_advisory_xact_lock(hashtext($1))`, [`db_code:${ddmmyy}`]);

  // Cari counter tertinggi untuk prefix hari ini.
  const { rows } = await client.query(
    `SELECT COALESCE(MAX(
       (regexp_replace(expense_code, '^DB-\\d{6}-', ''))::int
     ), 0) AS max_n
     FROM db
...
async function replacePivot(client, table, fkColumn, expenseId, ids) {
  await client.query(`DELETE FROM ${table} WHERE dbm_id = $1`, [expenseId]);
  ...
  await client.query(
    `INSERT INTO ${table} (dbm_id, ${fkColumn}) VALUES ${placeholders}`,
    params
  );
}
...
function buildWhere({ subEntities, poIds, invoiceIds, category1, dateFrom, dateTo }) {
  ...
  if (Array.isArray(poIds) && poIds.length > 0) {
    params.push(poIds);
    where.push(`EXISTS (
      SELECT 1 FROM db_purchase_orders epo
      WHERE epo.dbm_id = e.id AND epo.purchase_order_id = ANY($${params.length}::int[])
    )`);
  }
  ...
  if (Array.isArray(invoiceIds) && invoiceIds.length > 0) {
    ...
    where.push(`EXISTS (
      SELECT 1 FROM db_invoices ei
      WHERE ei.dbm_id = e.id AND ei.invoice_id = ANY($${params.length}::int[])
    )`);
  }
...
export default async function dbRoutes(fastify) {
  const authOnly = { preHandler: [fastify.authenticate] };
    const canViewPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('db', 'view')] };
const canCreate = { preHandler: [fastify.authenticate, fastify.requirePermission('db', 'create')] };
  const canEditPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('db', 'edit')] };
  const canDeletePerm = { preHandler: [fastify.authenticate, fastify.requirePermission('db', 'delete')] };
  ...
  fastify.post('/', canCreate, async (request, reply) => {
    ...
    try {
      ...
      await replacePivot(client, 'db_purchase_orders', 'purchase_order_id', newId, data.purchase_order_ids);
      await replacePivot(client, 'db_invoices',        'invoice_id',        newId, data.invoice_ids);
  ...
  fastify.put('/:id', canEditPerm, async (request, reply) => {
    ...
    try {
      ...
      await replacePivot(client, 'db_purchase_orders', 'purchase_order_id', id, data.purchase_order_ids);
      await replacePivot(client, 'db_invoices',        'invoice_id',        id, data.invoice_ids);
      await client.query('COMMIT');
      return { id };
    } catch (e) {
      await client.query('ROLLBACK');
  ...
  // ----- DELETE /expenses/:id -------------------------------
  // Hanya admin & superadmin. Pivot tables (db_purchase_orders,
  // db_invoices) sudah ON DELETE CASCADE dari sisi expense,
  ...
  fastify.delete('/:id', canDeletePerm, async (request, reply) => {
    ...
    const { rowCount } = await query(`DELETE FROM db WHERE id = $1`, [id]);
    if (rowCount === 0) {
      return reply.code(404).send({ error: 'Expense tidak ditemukan.' });
    }
    return { success: true };
    ...
    try {
      ...
      for (const r of validated) {
        ...
        if (r._poIds.length > 0) {
          await replacePivot(client, 'db_purchase_orders', 'purchase_order_id', newId, r._poIds);
        }
        if (r._invIds.length > 0) {
          await replacePivot(client, 'db_invoices', 'invoice_id', newId, r._invIds);
        }
      }
      await client.query('COMMIT');
