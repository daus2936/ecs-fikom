...
async function generateExpenseCode(client) {
  ...
  const prefix = `DBB-${ddmmyy}-`;

  // Advisory lock per-hari (key berbasis prefix tanggal).
  await client.query(`SELECT pg_advisory_xact_lock(hashtext($1))`, [`db2025_code:${ddmmyy}`]);

  // Cari counter tertinggi untuk prefix hari ini.
  const { rows } = await client.query(
    `SELECT COALESCE(MAX(
       (regexp_replace(expense_code, '^DBB-\\d{6}-', ''))::int
     ), 0) AS max_n
     FROM db_before_2025
...
const baseSchema = z.object({
  ...
  category1: z.literal('DB', { errorMap: () => ({ message: 'Kategori 1 harus "DB"' }) }),
...
function validateConsistency(data) {
  ...
  // Kategori 1 selalu "DB" (sudah dijamin schema). Tidak ada Kategori 3.
...
async function replacePivot(client, table, fkColumn, expenseId, ids) {
  await client.query(`DELETE FROM ${table} WHERE db_id = $1`, [expenseId]);
  ...
  await client.query(
    `INSERT INTO ${table} (db_id, ${fkColumn}) VALUES ${placeholders}`,
    params
  );
}
...
function buildWhere({ subEntities, poIds, invoiceIds, category1, dateFrom, dateTo }) {
  const where = [];
  const params = [];
  ...
  if (Array.isArray(poIds) && poIds.length > 0) {
    params.push(poIds);
    where.push(`EXISTS (
      SELECT 1 FROM db2025_purchase_orders epo
      WHERE epo.db_id = e.id AND epo.purchase_order_id = ANY($${params.length}::int[])
    )`);
  }
  ...
  if (Array.isArray(invoiceIds) && invoiceIds.length > 0) {
    ...
    where.push(`EXISTS (
      SELECT 1 FROM db2025_invoices ei
      WHERE ei.db_id = e.id AND ei.invoice_id = ANY($${params.length}::int[])
    )`);
  }
...
// Routes
// ============================================================
export default async function dbBefore2025Routes(fastify) {
  const authOnly = { preHandler: [fastify.authenticate] };
    const canViewPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('db-before-2025', 'view')] };
const canCreate = { preHandler: [fastify.authenticate, fastify.requirePermission('db-before-2025', 'create')] };
  const canEditPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('db-before-2025', 'edit')] };
  const canDeletePerm = { preHandler: [fastify.authenticate, fastify.requirePermission('db-before-2025', 'delete')] };

  // ----- GET /expenses-before-2025 --------------------------------------
  // Query params:
  ...
  fastify.get('/', canViewPerm, async (request, reply) => {
    ...
    const filters = { subEntities, poIds, invoiceIds, category1, dateFrom, dateTo };

    const { sql, params } = buildListQuery(filters);
    const { rows } = await query(sql, params);

    // Hitung total nominal dari SELURUH data yang cocok filter (tanpa LIMIT 500).
    ...
    return { db_before_2025: rows, summary };
  });

  // ----- GET /expenses-before-2025/:id ----------------------------------
  ...
  fastify.post('/', canCreate, async (request, reply) => {
    ...
    try {
      ...
      const { rows } = await client.query(
        `INSERT INTO db_before_2025
           (expense_code, occurred_date, expense_type, parent_company, sub_entity,
            category1, amount, notes, created_by, updated_by)
      ...
      await replacePivot(client, 'db2025_purchase_orders', 'purchase_order_id', newId, data.purchase_order_ids);
      await replacePivot(client, 'db2025_invoices',        'invoice_id',        newId, data.invoice_ids);
  ...
  fastify.put('/:id', canEditPerm, async (request, reply) => {
    ...
    try {
      ...
      await replacePivot(client, 'db2025_purchase_orders', 'purchase_order_id', id, data.purchase_order_ids);
      await replacePivot(client, 'db2025_invoices',        'invoice_id',        id, data.invoice_ids);
      await client.query('COMMIT');
      return { id };
    } catch (e) {
      await client.query('ROLLBACK');
  ...
  // ----- DELETE /expenses/:id -------------------------------
  // Hanya admin & superadmin. Pivot tables (db2025_purchase_orders,
  // db2025_invoices) sudah ON DELETE CASCADE dari sisi expense,
  ...
  fastify.delete('/:id', canDeletePerm, async (request, reply) => {
    ...
    const { rowCount } = await query(`DELETE FROM db_before_2025 WHERE id = $1`, [id]);
    if (rowCount === 0) {
      return reply.code(404).send({ error: 'Expense tidak ditemukan.' });
    }
    return { success: true };
  ...
  // ----- POST /expenses-before-2025/import ------------------------------
  ...
  fastify.post('/import', canCreate, async (request, reply) => {
    ...
    const importSchema = z.object({
      ...
      category1:     z.literal('DB', { errorMap: () => ({ message: 'category1 harus "DB"' }) }),
      amount: z.union([z.number(), z.string()])
        .transform((v) => (typeof v === 'string' ? Number(v) : v))
    ...
    try {
      ...
      for (const r of validated) {
        ...
        if (r._poIds.length > 0) {
          await replacePivot(client, 'db2025_purchase_orders', 'purchase_order_id', newId, r._poIds);
        }
        if (r._invIds.length > 0) {
          await replacePivot(client, 'db2025_invoices', 'invoice_id', newId, r._invIds);
        }
      }
      await client.query('COMMIT');
