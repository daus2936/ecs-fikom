...
const baseSchema = z.object({
  ...
  periode_pelunasan_hari:  z.union([z.number(), z.string()])
                            .transform((v) => parseInt(v, 10))
                            .refine((v) => Number.isInteger(v) && v > 0, 'Periode pelunasan harus > 0 hari'),
...
const LIST_SELECT = `
  SELECT bh.id, bh.tanggal_bayar, bh.nilai_bayar, bh.periode_pelunasan_hari,
  ...
  LEFT JOIN hutang   h  ON h.id  = bh.hutang_id
...
export default async function bayarHutangRoutes(fastify) {
  ...
  const adminOnly = { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'superadmin'])] };
    const canViewPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('bayar-hutang', 'view')] };
const canCreate = { preHandler: [fastify.authenticate, fastify.requirePermission('bayar-hutang', 'create')] };
  const canEditPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('bayar-hutang', 'edit')] };
  const canDeletePerm = { preHandler: [fastify.authenticate, fastify.requirePermission('bayar-hutang', 'delete')] };
  ...
  fastify.get('/', canViewPerm, async (request, reply) => {
    ...
    const sql = LIST_SELECT + whereClause
              + ' ORDER BY bh.tanggal_bayar DESC, bh.id DESC' + (isExport ? '' : ' LIMIT 10000');
    const { rows } = await query(sql, params);

    // Total nominal Nilai Bayar Hutang (tanpa LIMIT), ikut filter.
    // Join hutang karena filter kode_hutang memakai alias h.
    const sumSql = `SELECT COALESCE(SUM(bh.nilai_bayar), 0) AS total_amount, COUNT(*) AS cnt
                    FROM bayar_hutang bh LEFT JOIN hutang h ON h.id = bh.hutang_id${whereClause}`;
    ...
    return { bayar_hutang: rows, summary };
  });
  ...
  fastify.post('/', canCreate, async (request, reply) => {
    ...
    const { rows } = await query(
      `INSERT INTO bayar_hutang
        (tanggal_bayar, nilai_bayar, periode_pelunasan_hari,
         hutang_id, rekening_masuk_id, rekening_dari_id,
         created_by, updated_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$7)
       RETURNING id`,
      [d.tanggal_bayar, d.nilai_bayar, d.periode_pelunasan_hari,
       d.hutang_id, d.rekening_masuk_id, d.rekening_dari_id, request.user.id]
    );
  ...
  fastify.put('/:id', canEditPerm, async (request, reply) => {
    ...
    const { rowCount } = await query(
      `UPDATE bayar_hutang SET
         tanggal_bayar=$1, nilai_bayar=$2, periode_pelunasan_hari=$3,
         hutang_id=$4, rekening_masuk_id=$5, rekening_dari_id=$6, updated_by=$7
       WHERE id=$8`,
      [d.tanggal_bayar, d.nilai_bayar, d.periode_pelunasan_hari,
       d.hutang_id, d.rekening_masuk_id, d.rekening_dari_id, request.user.id, id]
    );
    if (rowCount === 0) return reply.code(404).send({ error: 'Bayar Hutang tidak ditemukan.' });
    return { id };
  });
  ...
  fastify.delete('/:id', canDeletePerm, async (request, reply) => {
    const id = Number(request.params.id);
    if (!Number.isInteger(id) || id <= 0) return reply.code(400).send({ error: 'ID tidak valid.' });
    const { rowCount } = await query(`DELETE FROM bayar_hutang WHERE id=$1`, [id]);
    if (rowCount === 0) return reply.code(404).send({ error: 'Bayar Hutang tidak ditemukan.' });
    return { success: true };
  });
}
