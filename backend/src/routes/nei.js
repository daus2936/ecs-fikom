...
async function replacePivot(client, table, fkColumn, parentId, ids) {
  await client.query(`DELETE FROM ${table} WHERE nei_id = $1`, [parentId]);
  if (ids.length === 0) return;
  const placeholders = ids.map((_, i) => `($1, $${i + 2})`).join(', ');
  await client.query(
    `INSERT INTO ${table} (nei_id, ${fkColumn}) VALUES ${placeholders}`,
    [parentId, ...ids]
  );
}
...
function buildWhereMulti({ subEntities, category3, nomorFaktur, poIds, invoiceIds, dateFrom, dateTo }) {
  ...
  if (Array.isArray(poIds) && poIds.length > 0) {
    params.push(poIds);
    where.push(`EXISTS (SELECT 1 FROM nei_purchase_orders icpo
                WHERE icpo.nei_id = c.id AND icpo.purchase_order_id = ANY($${params.length}::int[]))`);
  }
  if (Array.isArray(invoiceIds) && invoiceIds.length > 0) {
    params.push(invoiceIds);
    where.push(`EXISTS (SELECT 1 FROM nei_invoices ici
                WHERE ici.nei_id = c.id AND ici.invoice_id = ANY($${params.length}::int[]))`);
  }
...
function buildListQuery(filters, noLimit = false, sort = null) {
  ...
  if (sort === 'invoice_asc' || sort === 'invoice_desc') {
    ...
    orderBy = `(SELECT MIN(i.invoice_number) FROM nei_invoices ici
                JOIN invoices i ON i.id = ici.invoice_id
                WHERE ici.nei_id = c.id) ${dir} NULLS LAST, c.created_at DESC`;
  }
  const sql = `
...
async function insertOne(client, data, userId) {
  ...
  const id = rows[0].id;
  await replacePivot(client, 'nei_purchase_orders', 'purchase_order_id', id, data.purchase_order_ids);
  await replacePivot(client, 'nei_invoices',        'invoice_id',        id, data.invoice_ids);
  return id;
}
...
export default async function invoiceCoverRoutes(fastify) {
  ...
  const adminOnly = { preHandler: [fastify.authenticate, fastify.authorize(['admin', 'superadmin'])] };
    const canViewPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('nei', 'view')] };
const canCreate = { preHandler: [fastify.authenticate, fastify.requirePermission('nei', 'create')] };
  const canEditPerm = { preHandler: [fastify.authenticate, fastify.requirePermission('nei', 'edit')] };
  const canDeletePerm = { preHandler: [fastify.authenticate, fastify.requirePermission('nei', 'delete')] };
  ...
  fastify.get('/', canViewPerm, async (request, reply) => {
    ...
    return { nei: rows, summary };
  });

  // ----- GET /nei/faktur-pajak-options -----------
  ...
  fastify.put('/:id', canEditPerm, async (request, reply) => {
    ...
    try {
      ...
      const { rowCount } = await client.query(
        `UPDATE nei SET ${setCols} WHERE id = $${idIdx}`,
        params
      );
      if (rowCount === 0) {
  ...
  fastify.delete('/:id', canDeletePerm, async (request, reply) => {
    const id = Number(request.params.id);
    if (!Number.isInteger(id) || id <= 0) return reply.code(400).send({ error: 'ID tidak valid.' });
    const { rowCount } = await query(`DELETE FROM nei WHERE id = $1`, [id]);
